...
Status (2026-06-12): **Phase 5 as built is complete, pending explicit human
approval to advance.** The deterministic risk engine described above already
shipped in Phase 2 (`lib/risk`, byte-unchanged ever since). The human-approved
Phase 5 Master Brief redefined this phase's scope to the **AI Research
Copilot**: a draft-only advisory analyst layer (research questions, source
checklists, brief drafts, thesis critiques, missing-info analyses, skeptical
countercases) generated by a deterministic local fallback provider — no LLM
provider, no keys, no network — and stored in the `ai_research_drafts` ledger
entirely outside the risk path. Drafts never satisfy risk checks, never
promote sources/briefs/theses/settlement records, and never create paper
entries; risk verdicts are identical before and after drafting, proven by
dedicated isolation tests. Future automated web research / source discovery
may be built only on top of this draft boundary (AI output stays a draft a
human must separately act on) and still requires explicit human approval
before any phase adds it. See `docs/ARCHITECTURE.md`, `docs/RISK_ENGINE.md`
(Phase 5 status), `docs/SAFETY.md` (section i), and `docs/DECISION_LOG.md`.
