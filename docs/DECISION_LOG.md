...
2026-06-12 — Phase 5 AI draft ledger sits entirely outside the deterministic risk path — AI output is advisory research input, never a verdict or an authorization; letting generated drafts touch the risk path would violate the core principle (LLM recommends, rules permit) — Drafts in ai_research_drafts never satisfy risk checks, never promote sources/briefs/theses/settlement records to accepted/human_reviewed/ready_for_risk/human_verified, never create paper entries, and carry no verdict/stake/PnL fields; they are absent from ResearchSummary, ResearchStateResponse, toPersistedResearchSnapshot, and listResearchTickers.

2026-06-12 — Phase 5 ships a fallback-first deterministic provider with no real LLM and no keys — V1 Training Wheels forbids API keys/secrets in the repo, and a deterministic local provider keeps tests/builds network-free while proving the draft pipeline end-to-end — getAiResearchProvider() returns only the local_deterministic / phase5_fallback provider; no env reads, no network, no provider credentials anywhere; real providers slot in behind the AiResearchProvider interface only in a future explicitly approved phase.

2026-06-12 — AI research drafts have an archive-only lifecycle: no deletes, no unarchive — The store-wide audit philosophy (reject/archive, never delete) must cover AI artifacts too, so every generated draft remains reviewable — ai_research_drafts rows transition only from draft to archived; no delete function exists in the store module; archived drafts remain visible as an audit trail.

2026-06-12 — The forbidden-token safety scan is extended to lib/ai-research/** — Prompt/template text in the AI research module is generated-copy source material and must obey the same vocabulary discipline as routes and the research store — tests/safety-routes.test.ts scans app/api/**, lib/research-store/**, AND lib/ai-research/** with the unchanged FORBIDDEN_CODE_TOKEN_PATTERN and carve-out; the scan is strengthened, never widened.
